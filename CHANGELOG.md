...
- **DID Syntax Tightened to Spec Grammar** (review #4/L2): `did:plc` identifiers must be exactly 24 lowercase alphanumeric characters (previously any ≥8-char string, allowing traversal payloads like `did:plc:x/../../admin` through URL construction); `did:web` rejects userinfo (`@`) re-parsing (`trusted.example@evil.example` previously re-targeted resolution), empty segments, and structurally-invalid identifiers, while remaining spec-legal for `%`-encoded path segments (test environments encode ephemeral ports that way).
- **Discovered AS Endpoints Must Be HTTPS** (review M6): `authorization_endpoint`, `token_endpoint`, and `pushed_authorization_request_endpoint` are validated as HTTPS, hierarchical, and free of embedded credentials before use — a non-HTTPS `authorization_endpoint` would be copied into the user-agent redirect exposing authorization codes. Loopback/localhost HTTP is permitted to mirror `allow_insecure_localhost` test-mode semantics.
- **State-Store Admission Cap + Default Pruning** (review H5/M2): per-shard admission cap (`SHARD_ADMISSION_CAPACITY` = 1024 × 64 shards) fails closed with `StoreError::CapacityExceeded` after an opportunistic expired-entry prune — a PAR-flood attacker cycling login initiations can no longer grow memory unboundedly. The high-level client now also spawns TTL pruning for its default in-memory store (builder-provided stores keep caller-owned lifecycle).
- **`spawn_pruning_task` Precondition Assertions** (review M10): zero `interval_duration` and runtime-absence now panic with clean messages naming the invariant instead of tripping internal tokio panics.
- **Server-Side Cache-Exhaustion Defenses** (review H5): (1) the Tower middleware now commits replay-cache admission **only after** server-nonce and access-token validation succeed — attacker-minted proofs for invented tokens can no longer consume replay-cache capacity before their own validation (new two-phase `DPoPVerifier::verify_proof_deferred` + `commit_replay_admission` API; commit-time replay detection and saturation map to 401/503 respectively); (2) bare unauthenticated traffic (no DPoP proof, no `Authorization` header) receives a static 401 **without** fresh nonce allocation; (3) a **challenge-issuance rate limiter** (`CHALLENGE_ISSUANCE_RATE_LIMIT` = 64/second default, configurable via `InMemoryServerNonceSource::with_issuance_rate_limit`) bounds the cost of unauthenticated floods across all issuance paths, mapped to 503 — legitimate clients can no longer be displaced by a pre-authentication flood. Regression tests: invalid-proof flood leaves the replay cache empty; bare unauthenticated requests never receive nonces; rate limiter fails closed within its window.
