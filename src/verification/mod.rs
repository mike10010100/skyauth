...
//!
//! This module provides mathematical models, executable formal contracts,
...
//!
//! 1. **Executable Formal Contracts & Mathematical Transition Models ([`formal_models`])**:
...
//!      soundness.
//!    - Specifications for single-use OAuth state consumption, PKCE S256 bounds,
...
//! |---|---|---|---|
//! | **Single-Use State** | State token transitions from `Pending` to `Consumed` exactly once | [`formal_models::OAuthStateTransitionModel`] | [`kani_harnesses::proof_single_use_state_consumption`] |
//! | **SSRF Non-Bypassability** | No RFC 1918, link-local, or cloud metadata IP can pass filters | [`formal_models::SsrfFormalSpec`] | [`kani_harnesses::proof_ssrf_restricted_ip_rejection`] |
//! | **PKCE S256 Bounds** | $43 \le \text{len} \le 128$, unreserved character domain, 43-char challenge | [`formal_models::PkceFormalSpec`] | [`kani_harnesses::proof_pkce_s256_verifier_bounds`] |
//! | **Constant-Time Eq** | $\text{ct\_eq}(a, b) \iff a == b$ with data-independent execution time | [`formal_models::ConstantTimeEqSpec`] | [`kani_harnesses::proof_constant_time_eq_soundness`] |
//! | **DPoP HTU Invariants** | Strips query/fragment, normalizes case and default ports | [`formal_models::DPoPHtuFormalSpec`] | [`kani_harnesses::proof_dpop_htu_normalization_invariants`] |

pub mod formal_models;
pub mod kani_harnesses;

/// Compatibility alias for the formal models module.
pub use formal_models as verus_contracts;
